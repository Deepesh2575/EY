...
function ChatInterface() {
  ...
  const handleQuickStart = async (loanType: 'personal' | 'business' | 'home') => {
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
        e.preventDefault()
        const input = document.querySelector('textarea[placeholder*="Type your message"]') as HTMLTextAreaElement | null
        input?.focus()
      }
    }
      ...
      {currentStage === 'VIDEO_KYC' && !uploadedFiles['video_kyc_selfie'] && (
        <div className="absolute inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <VideoKYC
            ...
            onCancel={() => { /* Implement actual cancel logic later */ }}
          />
        </div>
      )}
